...
export default function ReviewPage() {
  ...
  // mutations
  const { mutate: submitGeneral, isPending: loadingGeneral } =
    useSubmitReviewMutation();
  const { mutate: submitLanguage, isPending: loadingLanguage } =
    useSubmitLanguageReviewMutation();
  ...
  const [languageForm, setLanguageForm] =
    useState<TSubmitLanguageReviewPayload>(() => ({
  ...
  const handleSubmit = () => {
    ...
    if (isLanguage) {
      submitLanguage({ id: videoId, data: languageForm }, { onSuccess });
    } else {
      submitGeneral({ id: videoId, data: generalForm }, { onSuccess });
    }
  };
          ...
          {isLanguage
            ...
            : generalCriteria.map(({ key, label }) => (
                <div key={key} className="space-y-1">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex-1 max-w-sm bg-blue-400 rounded-lg p-3">
                      <Slider
                        className="w-full"
